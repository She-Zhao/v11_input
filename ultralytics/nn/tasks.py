...
class BaseModel(nn.Module):
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if isinstance(x, dict):  # for cases of training and validating while training.
            return self.loss(x, *args, **kwargs)        # 训练的时候计算loss，否则直接预测
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        Args:
            ...
            visualize (bool): Save the feature maps of the model if True, defaults to False.
            embed (list, optional): A list of feature vectors/embeddings to return.
            embed（列表，可选）：要返回的特征向量/嵌入的列表。
        Returns:
            (torch.Tensor): The last output of the model.
        """
        y, dt, embeddings = [], [], []  # outputs
        for m in self.model:    # 这里仅是得到一个batch的预测结果
            if m.f != -1:  # if not from previous layer     选择输入,m.f就是yaml文件中网络每层的from，是一个int,代表网络来自哪一层
                # y是网络每一层的输出，这就是根据yaml中的from，从y中取出相应的网络输出，然后放在一个list中作为输入x
            ...
            if visualize:       # 如果 visualize=True，则调用 feature_visualization 函数保存该层的特征图,默认是Fasle
                feature_visualization(x, m.type, m.i, save_dir=visualize)
            if embed and m.i in embed:  # 若 embed 参数包含当前层索引 m.i，则使用自适应平均池化,试了下，训练时这部分没执行过
                embeddings.append(nn.functional.adaptive_avg_pool2d(x, (1, 1)).squeeze(-1).squeeze(-1))  # flatten
                if m.i == max(embed):   # 若 m.i 等于 embed 中的最大索引，则合并并返回 embeddings 列表中的所有嵌入向量。
    ...
    def loss(self, batch, preds=None):
        ...
        return self.criterion(preds, batch)     # 实际是调用了 self.criterion.__call__(preds, batch)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        # 通过全零输入的前向传播计算特征图的步幅。确保模型能够正确地推断输出特征图的大小
        m = self.model[-1]  # Detect()
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            s = 256  # 2x min stride
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3),verbose是否打印模型详细信息
    """Parse a YOLO model.yaml dictionary into a PyTorch model."""
    import ast

    # Args
    max_channels = float("inf")
    nc, act, scales = (d.get(x) for x in ("nc", "activation", "scales"))    # 从配置字典中提取目标检测的类别数、默认激活函数、尺度信息
    depth, width, kpt_shape = (d.get(x, 1.0) for x in ("depth_multiple", "width_multiple", "kpt_shape"))    # 获取深度和宽度的缩放系数
    if scales:      # 如果d中有scales
        scale = d.get("scale")
        if not scale:
            scale = tuple(scales.keys())[0]
            LOGGER.warning(f"WARNING ⚠️ no model scale passed. Assuming scale='{scale}'.")
        depth, width, max_channels = scales[scale]      # 根据 scale 值更新模型的 depth、width 缩放系数和 max_channels

    if act: # 使用 eval 动态解析激活函数的字符串值，将其赋给 Conv.default_act，设置为卷积层的默认激活函数
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args; f输入来源层的索引；n重复次数；m层的类型；args层的参数
        ...
        if m in {
            ...
            c1 = ch[f][-1] if isinstance(ch[f],list) else ch[f]
            c2 = args[0]     # f是来源层的索引，c1来自上一层的输出（初始默认是3），c2是yaml文件中args的第一个参数
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)如果c2不等于类别数（即Classify（）输出）
                c2 = make_divisible(min(c2, max_channels) * width, 8)   # 将通道数 c2 乘以width，并调整为 8 的倍数，以适应硬件的计算需
            if m is C2fAttn:    # args[1] 和 args[2] 是 C2fAttn 的特定参数，用于嵌入通道和头数。
                args[1] = make_divisible(min(args[1], max_channels // 2) * width, 8)  # embed channels 
                args[2] = int(
                    max(round(min(args[2], max_channels // 2 // 32)) * width, 1) if args[2] > 1 else args[2]
                )  # num heads  将args[1] args[2]调整为合理范围并与 width 缩放
            # args就是输入给模型每一个module的参数，下面这部分就是根据模型的不同，对args进行更改，同时更新维护c1c2
            args = [c1, c2, *args[1:]]  # 原本的args就是从yaml文件中读取的args，现在根据当前不同模块，对args进行添加 *agrs代表将原来args中第一个元素及以后的所有元素全部添加进来
            if m in {
                BottleneckCSP,
                C1,
                C2,
                ...
                args.insert(2, n)  # number of repeats 对特定模块，如 BottleneckCSP，在 args 中插入重复数 n，表示该模块的重复层数
                n = 1
            if m is C3k2 and scale in "mlx":  # for M/L/X sizes 如果模块是 C3k2，且模型尺度属于中、大或特大型（M/L/X），
                args[3] = True  # 则设置 args[3] = True，调整适配性
        elif m is AIFI:
            args = [ch[f], *args]
            ...
            c2 = args[1] if args[3] else args[1] * 4    # 对于 ResNetLayer，如果 args[3] 为 True，输出通道数 c2 为 args[1]，否则为 args[1] * 4
        elif m is nn.BatchNorm2d:
            args = [ch[f]]      # BatchNorm2d 模块仅需要输入通道数 ch[f]，所以直接设置为 args
        elif m is Concat:       
            c2 = sum(ch[x] for x in f)      # Concat 模块将多个通道合并，因此输出通道数 c2 为所有输入层通道的和
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect}:
            args.append([ch[x] for x in f])     # 将所有输入层的通道数列表添加到 args 中，供检测模块使用
            if m is Segment:
        ...
        """通过 m 和 args 实例化每个模块并将其添加到模型层列表中。"""
        # n是最开始从yaml读取的module重复的次数，若n>1，将 m(*args) 通过循环生成多个实例
        # n>1将多个实例使用 nn.Sequential 组合成一个顺序层（Sequential），n == 1，直接创建单一模块 m(*args)
        m_ = nn.Sequential(*(m(*args) for _ in range(n))) if n > 1 else m(*args)  # module m_ 是当前模块实例，可能包含单个或多个重复的子模块
        # 如果n>1，直接将m及其参数加入到容器中，否则将n个m及其参数加入到容器中
