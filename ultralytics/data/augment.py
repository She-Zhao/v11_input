...
class Compose:
    ...
    Attributes:
        ...
        transforms (List[Callable])：按顺序应用的转换函数列表。
    Methods:
        __call__: Applies a series of transformations to input data.    对输入数据应用一系列转换。
        append: Appends a new transform to the existing list of transforms.     将一个新的转换添加到现有的转换列表中。
        insert: Inserts a new transform at a specified index in the list of transforms. 在变换列表的指定索引处插入一个新的变换。
        __getitem__: Retrieves a specific transform or a set of transforms using indexing.  使用索引检索特定转换或一组转换。
        __setitem__: Sets a specific transform or a set of transforms using indexing.   使用索引设置一个或一组转换。
        tolist: Converts the list of transforms to a standard Python list.      将转换列表转换为标准的Python列表。
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        ...
        对图像进行缩放和填充（letterboxing），同时更新与图像相关的标签（如边界框、关键点等）
        ...
        if labels is None:
            labels = {}
        img = labels.get("img") if image is None else image     # 获取图像
        shape = img.shape[:2]  # current shape [height, width]  # 获取原始尺寸
        new_shape = labels.pop("rect_shape", self.new_shape)    # 计算新的目标尺寸
        ...
        # Compute padding   计算填充量
        ratio = r, r  # width, height ratios
        new_unpad = int(round(shape[1] * r)), int(round(shape[0] * r))
        dw, dh = new_shape[1] - new_unpad[0], new_shape[0] - new_unpad[1]  # wh padding
        if self.auto:  # minimum rectangle  处理自动填充和缩放填充
        ...
        if self.center:     # 处理中心对齐
            dw /= 2  # divide padding into 2 sides
            dh /= 2

        if shape[::-1] != new_unpad:  # resize  调整图像尺寸
            img = cv2.resize(img, new_unpad, interpolation=cv2.INTER_LINEAR)
        top, bottom = int(round(dh - 0.1)) if self.center else 0, int(round(dh + 0.1))  # 添加填充
    ...
    def _update_labels(self, labels, ratio, padw, padh):
        ...
        在图像缩放和填充之后，更新标签中边界框的位置和大小，以确保这些信息与新的图像尺寸匹配
...
class Format:
    ...
    处理图像数据和标签，以便模型训练或推理时使用
    This class standardizes image and instance annotations to be used by the `collate_fn` in PyTorch DataLoader.
    这个类对PyTorch数据加载器中的`collate_fn`使用的图像和实例注释进行标准化。
    Attributes:
    ...
    def __init__(
        ...
        为模型（尤其是目标检测和实例分割）预处理图像和标签数据。并对图像和标签进行格式转换、归一化、掩码生成等操作。
        Args:
    ...
    def __call__(self, labels):
        ...
        接收包含图像和标签的字典并对其进行处理，返回处理后的图像和标签。
    ...
    def _format_img(self, img):
        ...
        _format_img 用于对图像进行预处理，将图像格式从 HWC 转为 CHW，同时转换为tensor，并可能随机翻转颜色通道
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    """
    Applies a series of image transformations for training.     应用一系列图像变换进行训练。

    This function creates a composition of image augmentation techniques to prepare images for YOLO training.
    It includes operations such as mosaic, copy-paste, random perspective, mixup, and various color adjustments.
    该函数创建一个图像增强技术的组合，为YOLO训练准备图像。
    Args:
    ...
    """
    mosaic = Mosaic(dataset, imgsz=imgsz, p=hyp.mosaic) # Mosaic 数据增强操作
    affine = RandomPerspective(                         # 几何变换
        degrees=hyp.degrees,
    ...
    pre_transform = Compose([mosaic, affine])   # 初步的数据增强组合。按照顺序应用Mosaic 和 RandomPerspective
    if hyp.copy_paste_mode == "flip":   # 根据 hyp.copy_paste_mode 的值，决定是否使用水平翻转的 CopyPaste 模式
    ...
    if dataset.use_keypoints:   # 如果数据集包含关键点（例如姿态估计），函数会检查 flip_idx（翻转索引）
    ...
    return Compose( # 返回一个包含多个增强操作的 Compose 对象。这个对象会依次应用 pre_transform（ Mosaic、RandomPerspective、CopyPaste）
        [   # 以及其他操作如 MixUp（混合图像）、Albumentations（开源增强库的增强操作）、RandomHSV（色彩变化）、RandomFlip（随机翻转）等
            pre_transform,
            MixUp(dataset, pre_transform=pre_transform, p=hyp.mixup),
            Albumentations(p=1.0),
            # RandomHSV(hgain=hyp.hsv_h, sgain=hyp.hsv_s, vgain=hyp.hsv_v),    # 20241107修改
