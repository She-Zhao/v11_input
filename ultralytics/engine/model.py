...
class Model(nn.Module):
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        """
        cfg_dict = yaml_model_load(cfg)     # 从指定的 YAML 文件加载的 YOLOv8 模型的配置信息，以字典d存储
        self.cfg = cfg
    ...
    def train(
        ...
        **kwargs,
        ...
        # 得到self.trainer = self.task_map[detect][trainer] = yolo.detect.DetectionTrainer
        self.trainer = (trainer or self._smart_load("trainer"))(overrides=args, _callbacks=self.callbacks) 
        if not args.get("resume"):  # manually set model only if not resuming
            """
            此处比较重要,为开始定义我们的对应任务的模型了，比如我这里task设置的为Detect,那么此处会实例化DetectModel模型。
        ...
        """训练的核心函数，self.trainer.train()"""
        self.trainer.train()                    # 重点，这里就是进行训练的地方
